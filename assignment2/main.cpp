...
void Johnson_Trotter(const unsigned int n)
{
    if (n != 0)
    {
        // create 2D array rows
        int **directionAndArray = new int *[2];
        // allocate coloums in 2D array
        for (int i = 0; i < 2; i++)
        {
        ...
        int biggestMobile;
        int totalPermutations = 1; // used to print the amount of permutation there are in the set
        int temp;                  // for swaping later

        // calculate factorial to find the permutations
        for (int i = 1; i < n + 1; i++)
        {
            totalPermutations *= i;
        }
    ...
    {
        cout << "That is not a valid set for the Johnson-Trotter Algorithm" << endl;
    }
}
